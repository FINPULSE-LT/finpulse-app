...
export const LandingScreen: React.FC<LandingScreenProps> = ({
    ...
    try {
      ...
      if (error) {
        if (
          ...
          setErrorMessage(
            "El inicio con Google requiere activar el proveedor en Supabase. Ingresa directamente con tu contraseña o en Modo Demo."
          );
        } else {
  ...
  return (
    <div className="min-h-screen bg-[#06110D] text-white flex flex-col selection:bg-[#00F5A0] selection:text-slate-950">
      {/* Barra Superior con Logo y Acceso Rápido */}
      <header className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 flex items-center justify-between border-b border-emerald-900/40">
      ...
      <main className="flex-1 flex flex-col justify-center max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-10 lg:py-16">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
          ...
          {/* Columna Derecha: Tarjeta de Inicio de Sesión Simplificada */}
          <div className="lg:col-span-5">
            <div className="relative rounded-3xl p-6 sm:p-8 bg-gradient-to-b from-[#0D1C17] via-[#091512] to-[#060D0B] border border-[#00F5A0]/30 shadow-[0_0_50px_rgba(0,245,160,0.15)] space-y-5">
              ...
              <form onSubmit={handlePasswordLogin} className="space-y-3.5">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-9 pr-10 py-2.5 bg-[#060E0C] border border-slate-700/80 rounded-xl text-xs text-white focus:outline-none focus:border-[#00F5A0] transition-colors font-mono"
                    />
                    <button
                      type="button"
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className="w-full py-3 px-4 rounded-xl bg-gradient-to-r from-[#00F5A0] to-[#00D9F5] text-slate-950 font-black text-xs hover:opacity-95 transition-all shadow-md active:scale-98 disabled:opacity-50 cursor-pointer"
                >
                  {isLoading ? "Validando credenciales..." : "Ingresar a mi Cuenta"}
                </button>
              </form>
              ...
              <button
                ...
                disabled={isLoading}
                className="w-full py-2.5 px-4 bg-[#0A1613] hover:bg-[#0E201B] border border-slate-700/80 text-white rounded-xl font-bold text-xs flex items-center justify-center gap-2.5 transition-all shadow-sm active:scale-98 cursor-pointer"
              >
              ...
              <div className="pt-2 border-t border-slate-800/80 space-y-1.5">
                <button
                  type="button"
                  onClick={onEnterDemo}
                  className="w-full py-2.5 px-3 rounded-xl bg-[#091512] hover:bg-[#0E201B] border border-[#00F5A0]/40 text-[#00F5A0] font-bold text-xs flex items-center justify-center gap-2 transition-all shadow-inner hover:border-[#00F5A0] active:scale-98"
                >
                  <Sparkles className="w-3.5 h-3.5" />
